...
#define IMX219_OFILM_180_OTP_VCM_OFFSET_VALUE 200
typedef enum {
...
static bool imx219_otp_read_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	vendor_id = buf[4]>>4;
	if(0 == strncmp(s_ctrl->sensordata->sensor_name , "imx219_ofilm_att", strlen(s_ctrl->sensordata->sensor_name)))
	{
		if (vendor_id == OFILM_180_MODULE_VENDOR_ID && buf[3] == 0xb4) { //(180 = 0xb4)
			imx219_otp_flag |= IMX219_OTP_ID_READ;
			imx219_otp_vendor_module = OFILM_180_MODULE_VENDOR_ID;
			return true;
		} else {
...
static bool imx219_otp_read_vcm(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	switch (imx219_otp_vendor_module)
	{
		...
		case OFILM_180_MODULE_VENDOR_ID:
		{
		    CMR_LOGW("imx219 otp is ofilm 180 module!\n");
			vcm_offset_value = IMX219_OFILM_180_OTP_VCM_OFFSET_VALUE;
			break;
		}
		default:
		{
