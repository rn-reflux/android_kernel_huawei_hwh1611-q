...
static int fem_check_5g_flag_show(struct seq_file *m, void *v)
{
     ...
     {
         printk("%s enter fem_check_5g_flag:%s;\n", __func__,fem_check_5g_flag);
     }
     else
     {
         printk("%s get fem_check_5g_flag failed;\n", __func__);
         return 0;
     }
...
static int __init wlanfeaturetrans_init(void)
{
    ...
    if (!ent)
    {
        printk("%s:Unable to create /proc/%s/fem_check_5g_flag entry",__func__,PROC_DIR);
        retval =  -ENOMEM;
