	...
	for(j = 0;j < ctrl->esd_cmds.cmd_cnt;j++)
	{
		...
		do
		{
			...
			for(i=0;i<(ctrl->esd_cmds.cmds[j].dchdr.dlen-1);i++)
			{
				...
				if((ctrl->esd_cmds.cmds[j].payload[0] == 0x52))
				{
					if((esd_buf[i] == 0x00)&&(ctrl->panel_data.panel_info.esd_bl_check != 0x00))
					{
						compare_reg = -1;
						lcd_report_dsm_err(DSM_LCD_ESD_STATUS_ERROR_NO,esd_buf[i] , ctrl->esd_cmds.cmds[j].payload[0]);
						LCD_LOG_ERR("panel ic error: in %s,  reg 0x52 esd error\n",__func__);
					}
					break;
				}
...
/* Modify JDI tp/lcd power on/off to reduce power consumption */
	...
	rc = of_property_read_u32(np, "huawei,two-reset-enable", &tmp);
	pinfo->two_reset_enable = (!rc ? tmp : 0);
