...
static long release_wakelock;
static bool usb_is_in_factory_mode = false;
...
{
    dev_info(the_msm_otg->phy.dev, "[USB_DEBUG]: Enter %s, usb_is_in_factory_mode is %d!\n", __func__, usb_is_in_factory_mode);

    if (usb_is_in_factory_mode)
    {
         ...
         struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if (usb_is_in_factory_mode)
    {
        ...
        {
            dev_info(the_msm_otg->phy.dev, "[USB_DEBUG]: Enter %s, return  -EINVAL!\n", __func__);
            return -EINVAL;
        }
        ...
        if (1 == release_wakelock)
        {
            ...
            charge_into_sleep_mode(0);
        }
        else
        {
            if (!wake_lock_active(&the_msm_otg->wlock))
            {
                dev_info(the_msm_otg->phy.dev, "[USB_DEBUG]: usb dev_wakelock lock.\n");
                msm_otg_set_vbus_state(1);
		        charge_into_sleep_mode(1);
            }
        }
        return size;
    }
