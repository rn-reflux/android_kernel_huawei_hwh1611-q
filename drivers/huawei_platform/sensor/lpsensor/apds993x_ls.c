...
static unsigned int apds993x_ppcount_value = 0xC;
			...
			else
			{
				switch(tp_type)
				{
					...
					case FW_GIS:
						...
						break;
					case FW_TOPTOUCH:
						rc = apds993x_get_als_array(np,"avago,als_array_toptouch");
						break;
					default:
						rc = -EINVAL;
...
static int apds993x_init_client(struct i2c_client *client)
{
	...
	err = apds993x_set_ppcount(client, apds993x_ppcount_value);
		...
		struct apds993x_platform_data *pdata)
{
	...
	pdata ->pwindow= tmp;
	rc = of_property_read_u32(np, "avago,ppcount", &tmp);
	if (rc) {
		apds993x_ppcount_value = 0xC;
	}
	else
	{
	      apds993x_ppcount_value = tmp;
	}
