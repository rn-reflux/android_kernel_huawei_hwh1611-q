...
static char *generate_firmware_filename(struct device *dev)
{
	...
	char *filename = NULL;
	char lcd_panel_name[LCD_PANEL_NAME_LEN] = {0};
	int ret = 0;

	if (core_pdata->fw_name_depend_lcd) {
		ret = get_lcd_panel_name(lcd_panel_name, LCD_PANEL_NAME_LEN);
		if (ret) {
			tp_log_err("%s %d:get lcd panel name fail\n", __func__, __LINE__);

#ifdef CONFIG_HUAWEI_DSM
			// if get lcd panel name fail, report dmd
			cyttsp5_tp_report_dsm_err(dev, DSM_TP_GET_LCD_PANEL_NAME_ERROR_NO, 0);
#endif/*CONFIG_HUAWEI_DSM*/

			return NULL;
		}
		tp_log_info("%s:lcd panel name = %s\n", __func__, lcd_panel_name);
	}
	...
	if (core_pdata->fw_name_depend_lcd) {
		snprintf(filename, FILENAME_LEN_MAX, "%s_%s_fw.bin",
			product_name, lcd_panel_name);
	} else {
		snprintf(filename, FILENAME_LEN_MAX, "%s_fw.bin",
			product_name);
	}
#else /* CONFIG_HUAWEI_KERNEL */
	panel_id = cyttsp5_get_panel_id(dev);
