...
ssize_t cyttsp5_dsm_record_get_lcd_panel_err_info( void )
{
	ssize_t size = 0;
	ssize_t total_size = 0;
	const char * lcd_panel_info = NULL;

	tp_log_info("%s: entry!\n", __func__);

	lcd_panel_info = get_lcd_panel_info();
	if (NULL == lcd_panel_info) {
		lcd_panel_info = "get lcd panel info fail";
	}

	size = dsm_client_record(tp_cyp_dclient,
		"get lcd panel name fail, lcd panel info is:%s\n", lcd_panel_info);
	total_size += size;
...
int cyttsp5_tp_report_dsm_err(struct device *dev, int type, int err_numb)
{
	...
	switch(type)
	{
		...
		case DSM_TP_ESD_ERROR_NO:
			...
			break;
		case DSM_TP_GET_LCD_PANEL_NAME_ERROR_NO:
			cyttsp5_dsm_record_get_lcd_panel_err_info();
			break;
		default:
			break;
	}
