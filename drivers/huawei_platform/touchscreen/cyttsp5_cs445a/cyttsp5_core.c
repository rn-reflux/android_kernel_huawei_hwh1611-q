...
again:
	...
	read_len = get_unaligned_le16(&cd->response_buf[7]);
	tp_log_debug("%s:current read_len = %d, target read_length = %d, total_read_len = %d\n",
...
static void inline process_charger_armor_level(struct cyttsp5_core_data *cd)
{
	...
	if (cd->last_charger_armor_level != charger_armor_level)
		tp_log_info("%s: charger armor level changed : %d to %d\n",
				...
				char *buf, size_t size, u8 data_id)
{
	...
	if (rc < 0) {
		tp_log_err("%s(%d): Error on retrieve panel scan r=%d\n",
			   __func__, __LINE__, rc);
		return rc;
	}
	...
	while (actual_read_len > 0) {
		cyttsp5_hid_output_retrieve_panel_scan_(cd, elem_offset, 0xFFFF,
					data_id, NULL,
		...
		elem_offset += actual_read_len;
		if (read_len >= size) {
			tp_log_err("%s:read buf overflow, have read %d\n", __func__, read_len);
			break;
		}
	}
				...
				char *buf, size_t size, u8 data_type)
{
	...
	switch (data_type) {
		...
		case CY_PANEL_SCAN_BASLINE:
			...
			break;
		default:
			tp_log_err("%s: invild data type : %d\n", __func__, data_type);
			return -EINVAL;
	}
	...
	if (!input_buf || !mutual_raw_data || !self_cap_raw_data) {
		tp_log_err("%s:out of memnry\n", __func__);
		goto exit;
	}

	rc = cyttsp5_hid_output_exec_panel_scan_(cd);
	if (rc) {
		tp_log_err("%s:exec panel scan fail\n", __func__);
		goto exit;
	}
	...
	rc = cyttsp5_receive_panel_scan_data(dev, input_buf, tx_num * rx_num * 2,
											mutual_data_id);
	if (rc) {
		tp_log_err("%s:get mutual panel_scan_data fail,ret = %d\n", __func__, rc);
		goto exit;
	}
	...
	rc = cyttsp5_receive_panel_scan_data(dev, input_buf, (tx_num + rx_num) * 2,
											self_cap_data_id);
	if (rc) {
		tp_log_err("%s:get self panel_scan_data fail,ret = %d\n", __func__, rc);
		goto exit;
	}
					...
					struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = kstrtoul(buf, 10, &value);
	if (ret < 0) {
		tp_log_err("%s: input value invaild\n", __func__);
		return ret;
	}

	dad->touch_debug_data_type = value;
	tp_log_info("%s:input value = %d\n", __func__, (int)value);
	return size;
}
					 ...
					 struct device_attribute *attr, char *buf)
{
	...
	if (size < 0)
		tp_log_err("%s:cyttsp5_receive_panel_scan fail, return size = %d\n",
						__func__, rc);
	...
	rc = cyttsp5_hid_output_resume_scanning_(cd);
	if (rc < 0)
			tp_log_err("%s: Error on resume scanning rc=%d\n", __func__, rc);
					 ...
					 struct device_attribute *attr, char *buf)
{
	...
	rc = cyttsp5_hid_send_output_and_wait_(cd, &hid_output);
	if (rc < 0) {
		tp_log_err("%s: exec command fail, rc = %d\n", __func__, rc);
		return rc;
	}
...
static ssize_t ts_capacitance_test_type_show(struct kobject *dev,
						struct kobj_attribute *attr, char *buf)
{
	struct device *cdev = cyttsp5_core_dev;
	struct cyttsp5_core_data *cd = dev_get_drvdata(cdev);
	return snprintf(buf, PAGE_SIZE, "%s\n", 
						cd->cpdata->tp_test_type ? cd->cpdata->tp_test_type : "Normalize_type:judge_last_result");
}

static struct kobj_attribute tp_capacitance_test_type = {
	.attr = {.name = "tp_capacitance_test_type", .mode = S_IRUGO},
	.show = ts_capacitance_test_type_show,
	.store = NULL,
};
...
static int add_cap_test_support_info_sysfs(struct device *dev)
{
	int error = 0;
	struct kobject *properties_kobj;

	properties_kobj = kobject_create_and_add("touchscreen", NULL);
	if(NULL == properties_kobj) {
		tp_log_err("%s: create kobject touchscreen fail!\n", __func__);
		return -1;
	}
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	rc = add_cap_test_support_info_sysfs(cd->dev);
	if (rc < 0) {
		tp_log_err( "%s: Error, cap test support info sysfs fail! \n", __func__);
	}
	gdev = dev;
