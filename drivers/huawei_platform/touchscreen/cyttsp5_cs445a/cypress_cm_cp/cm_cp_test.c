					...
					int32_t *raw_data)
{
	...
	if (read_length != actual_read_length) {
		tp_log_err("%s:Read length is not match!\n", __func__);
		ret = -EINVAL;
		goto free;
	}
	...
	if (element_size != 1 && element_size != 2 && element_size != 4) {
		tp_log_err("%s:Element size error!\n", __func__);
		ret = -EINVAL;
		goto free;
	}
				...
				int gidac_mult, int clk, int vtx, int mtx_sum)
{
	...
	return i_bias * adc / (clk * vtx * mtx_sum);	
}
							...
							int *cm_sensor_average)
{
	...
	*cm_sensor_data = kzalloc(sensor_element_num * sizeof(int), GFP_KERNEL);
	...
	tp_log_debug("%s: sensor_raw_data[0..%d]:\n", __func__,
			sensor_element_num - 1);
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			tp_log_debug("%5d ", (*sensor_raw_data)[i * rx_num + j]);
		}
		tp_log_debug("\n");
	}
	...
	*cm_sensor_average /= sensor_element_num;
	...
	tp_log_debug("%s: cm_sensor_data[0..%d]:\n", __func__,
			sensor_element_num - 1);
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			tp_log_debug("%d ", (*cm_sensor_data)[i * rx_num + j]);
		}
		tp_log_debug("\n");
	}
							...
							int *cm_sensor_average,int *cm_button_average)
{
	...
	*cm_sensor_data = kzalloc(sensor_element_num * sizeof(int), GFP_KERNEL);
	...
	tp_log_info("%s: hw_bl_gidac_mutual: %d\n", __func__, hw_bl_gidac_mutual);
	tp_log_info("%s: hw_bl_pwc_mutual: %d\n", __func__, hw_bl_pwc_mutual);
	...
	ret = get_configuration_parameter(HW_BL_GIDAC_LSB_CONFIG,
				&parameter_type, &parameter_value);
	if (ret || parameter_type != INTEGER) {
		tp_log_err("%s:Unable to get hw_bl_gidac_lsb_config!\n", __func__);
		goto restore_multi_tx;
	}
	hw_bl_gidac_lsb_config = (uint32_t)parameter_value.integer;
	tp_log_info("%s: hw_bl_gidac_lsb_config:%d\n", __func__, hw_bl_gidac_lsb_config);
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			tp_log_debug("sen[%d][%d] = %5d ", i, j, (*sensor_raw_data)[i * rx_num + j]);
		}
		tp_log_debug("\n");
	}
	...
	/* Step 5 and 6: Calculate Cm_sensor and Cm_sensor_ave */
	*cm_sensor_average = 0;
	tp_log_info("%s: calculate_cm_gen6l, sensor_element_num = %d\n", __func__, sensor_element_num);
	for (i = 0; i < sensor_element_num; i++) {
					...
					int clk, int vtx, int mtx_sum)
{
	...
	return i_bias * t_cal * rx_atten_mutual / (vtx * mtx_sum);
}
					...
					int **cm_sensor_row_delta, bool *pass)
{
	...
	*cm_sensor_row_delta =
	...
	if (cm_sensor_calibration < configuration->cm_min_limit_cal
		|| cm_sensor_calibration > configuration->cm_max_limit_cal) {
		tp_log_err("%s: Cm_sensor_calibration:%d (%d,%d)\n",
				"Cm sensor Cm_cal min/max test",
				cm_sensor_calibration,
				configuration->cm_min_limit_cal,
				configuration->cm_max_limit_cal);
								...
								*cp_sensor_tx_calibration)
{
	...
	*cp_sensor_tx_calibration =
	...
	tp_log_debug("%s: cp_sensor_rx_raw_data[0..%d]:\n", __func__, rx_num - 1);
	for (i = 0; i < rx_num; i++) {
		tp_log_debug("%5d ", (*sensor_rx_raw_data)[i]);
	}
	tp_log_debug("\n");
	...
	*cp_sensor_rx_average /= rx_num;
	...
	tp_log_debug("%s: cp_sensor_rx_data[0..%d]:\n", __func__, rx_num - 1);
	for (i = 0; i < rx_num; i++) {
		tp_log_debug("%d ", (*cp_sensor_rx_data)[i]);
	}
	tp_log_debug("\n");
	...
	tp_log_debug("%s: cp_sensor_tx_raw_data[0..%d]:\n", __func__, tx_num - 1);
	for (i = 0; i < tx_num; i++) {
		tp_log_debug("%5d ", (*sensor_tx_raw_data)[i]);
	}
	...
	*cp_sensor_tx_average /= tx_num;
	...
	tp_log_debug("%s: cp_sensor_tx_data[0..%d]:\n", __func__, tx_num - 1);
	for (i = 0; i < tx_num; i++) {
		tp_log_debug("%d ", (*cp_sensor_tx_data)[i]);
	}
	tp_log_debug("\n");
								...
								int *cp_button_calibration)
{
	...
	int v_swing;
	uint32_t tx_period_self;
	uint32_t scaling_factor_self_rx;
	uint32_t scaling_factor_self_tx;
	uint32_t balancing_target_self;
	uint32_t gidac_mult;
	int idac_lsb = IDAC_LSB_DEFAULT;
	int clk = CLK_DEFAULT;
	...
	tp_log_info("%s: hw_bl_gidac_self_rx=%d, hw_bl_gidac_self_tx = %d\n", __func__,hw_bl_gidac_self_rx,hw_bl_gidac_self_tx);
	...
	for(i=0; i<rx_num; i++)
		tp_log_debug("%s: hw_bl_pwc_self_rx[%d]: %d\n", __func__, i, hw_bl_pwc_self_rx[i]);

	for(i=0; i<tx_num; i++)
		tp_log_debug("%s: hw_bl_pwc_self_tx[%d]: %d\n", __func__, i, hw_bl_pwc_self_tx[i]);
	...
	*cp_sensor_tx_calibration = kzalloc(tx_num * sizeof(int), GFP_KERNEL);
	...
	/* Get CDC:SCALING_FACTOR_SELF_RX */
	ret = get_configuration_parameter(SCALING_FACTOR_SELF_RX, &parameter_type,
						&parameter_value);
	if (ret || parameter_type != INTEGER) {
		tp_log_err("%s:Unable to get scaling_factor_self_rx!\n", __func__);
		goto free_buffers;
	}

	scaling_factor_self_rx = parameter_value.integer;
	tp_log_info("%s: scaling_factor_self_rx: %d\n", __func__,
			scaling_factor_self_rx);

	/* Get CDC:SCALING_FACTOR_SELF_TX */
	ret = get_configuration_parameter(SCALING_FACTOR_SELF_TX, &parameter_type,
				&parameter_value);
	if (ret || parameter_type != INTEGER) {
		tp_log_err("%s:Unable to get scaling_factor_self_tx!\n", __func__);
		goto free_buffers;
	}

	scaling_factor_self_tx = parameter_value.integer;
	tp_log_info("%s: scaling_factor_self_tx: %d\n", __func__,
			scaling_factor_self_tx);
	...
	tp_log_debug("%s: cp_sensor_rx_calibration[0..%d]:\n", __func__, rx_num - 1);
	for (i = 0; i < rx_num; i++){
		tp_log_debug("%5d  ", hw_bl_pwc_self_rx[i]);
		tp_log_debug("%5d\n", (*cp_sensor_rx_calibration)[i]);
	}
	tp_log_debug("\n");

	tp_log_debug("%s: cp_sensor_tx_calibration[0..%d]:\n", __func__, tx_num - 1);
	for (i = 0; i < tx_num; i++){
		tp_log_debug("%5d  ", hw_bl_pwc_self_tx[i]);
	...
	*cp_sensor_rx_average = 0;
	for (i = 0; i < rx_num; i++) {
		(*cp_sensor_rx_data)[i] = calculate_cp_gen6l((*sensor_rx_raw_data)[i],
				...
				scaling_factor_self_rx, idac_lsb, idac_self_rx,
				rx_atten_self_rx, gidac_mult, clk, v_swing,
				hw_bl_gidac_lsb_config, hw_bl_gidac_self_rx, hw_bl_pwc_self_rx[i]);
	...
	*cp_sensor_tx_average = 0;
	...
	for (i = 0; i < tx_num; i++) {
		(*cp_sensor_tx_data)[i] = calculate_cp_gen6l((*sensor_tx_raw_data)[i],
				tx_period_self, balancing_target_self,
				scaling_factor_self_tx, idac_lsb, idac_self_tx,
				rx_atten_self_tx, gidac_mult, clk, v_swing,
				hw_bl_gidac_lsb_config, hw_bl_gidac_self_tx, hw_bl_pwc_self_tx[i]);
	...
	*cp_sensor_tx_average = cp_sensor_average_bak / tx_num;
	...
	tp_log_debug("%s: sensor_rx_raw_data[0..%d]:\n", __func__, rx_num - 1);
	for (i = 0; i < tx_num; i++){
		tp_log_debug("%5d ", (*sensor_rx_raw_data)[i]);
	}
	tp_log_debug("\n");

	tp_log_debug("%s: sensor_tx_raw_data[0..%d]:\n", __func__, tx_num - 1);
	for (i = 0; i < tx_num; i++)
		tp_log_debug("%5d ", (*sensor_tx_raw_data)[i]);
	tp_log_debug("\n");
	...
	tp_log_debug("%s: cp_sensor_tx_data[0..%d]:\n", __func__, tx_num - 1);
	for (i = 0; i < tx_num; i++){
		tp_log_debug("%5d ", (*cp_sensor_tx_data)[i]);
	}
	tp_log_debug("\n");

	tp_log_debug("%s: cp_sensor_rx_data[0..%d]:\n", __func__, rx_num - 1);
	for (i = 0; i < rx_num; i++)
		tp_log_debug("%5d ", (*cp_sensor_rx_data)[i]);
	tp_log_debug("\n");
					...
					bool *pass)
{
	...
	for(i=0; i<rx_num; i++) {
		if(cp_sensor_rx_delta[i] > configuration->cp_max_delta_sensor_rx_percent) {
			tp_log_err("%s: Cp_sensor_rx_delta:%2d(%d)\n",
	...
	for(i=0; i<tx_num; i++) {
		if(cp_sensor_tx_delta[i] > configuration->cp_max_delta_sensor_tx_percent) {
			tp_log_err("%s: Cp_sensor_tx_delta:%2d(%d)\n",
				"Cp sensor delta range limit test",
					...
					struct result *result,bool *pass)
{
	...
	if (configuration_rx_num != rx_num || configuration_tx_num != tx_num) {
		tp_log_err("%s:config file may mismatch, %s=%d, %s=%d, rx_num=%d, tx_num=%d\n",
					__func__,
					...
					struct result *result, bool *pass)
{
	...
	*cm_sensor_row_delta = kzalloc((tx_num * rx_num) * sizeof(int), GFP_KERNEL);
	...
	if (configuration_sensor_num != sensor_num) {
		tp_log_err("%s: %s (Configuration:%d, Device:%d)\n", __func__,
			...
			bool run_cp_test, bool *cm_test_pass, bool *cp_test_pass)
{
	...
	if (ret) {
		...
		tp_log_info("%s: short test fail cause of software reason\n", __func__);
		seq_printf(result_file, "2F-");
	} else {
	...
	if (run_cp_test) {
		...
		if (!skip_cp_sensor) {
			if(g_family_type == 2) {
				...
				tp_log_debug("%s: cp_sensor_rx_delta_gen6l[0..%d]: ", __func__, rx_num-1);
				for (i = 0; i < rx_num; i++)
					tp_log_debug("%5d\n", cp_sensor_rx_delta_gen6l[i]);
				tp_log_debug("\n");
				tp_log_debug("%s: cp_sensor_tx_delta_gen6l[0..%d]: ", __func__, tx_num-1);
				for (i = 0; i < tx_num; i++)
