    ...
    switch(info->name)
    {
        ...
        case CHARGE_SYSFS_IIN_RUNNINGTEST:
        {
            if(di->batt_psy && di->batt_psy->get_property)
            {
                ...
                pmu_log_info("IIN_RUNNINGTEST get chargeenable val = %d\n",val.intval);
            }
...
static ssize_t charge_sysfs_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    switch(info->name)
    {
        ...
        case CHARGE_SYSFS_IIN_RUNNINGTEST: /* running test charging/discharge function*/
        {
            if((strict_strtol(buf, 10, &val) < 0)||(val < 0)||(val > 3000))
                return -EINVAL;
            if(di->batt_psy && di->batt_psy->set_property)
            {
                pmu_log_info("IIN_RUNNINGTEST set chargeenable val = %ld\n",val);
                ret.intval = (int)val;
                di->batt_psy->set_property(di->batt_psy,POWER_SUPPLY_PROP_CHARGING_ENABLED,&ret);
            }
            pmu_log_info("IIN_RUNNINGTEST set chargeenable ret.intval = %d\n",ret.intval);
...
void charge_into_sleep_mode(int value)
{
    struct charge_device_info *di = NULL;
    ...
    if(NULL == di)
    {
        pr_err("set charge ic into sleep mode failed!\n");
        return ;
    }
    ...
    if(true == charger_factory_flag)
    {
        ...
        if(1 == value)
        {
            if (wake_lock_active(&di->chg_wake_lock))
            {
        ...
        di->ops->set_chargeic_into_sleep_mode(value);
    }
    else
    {
        pr_err("charger_factory_flag is %d!\n",charger_factory_flag);
...
static int charge_probe(struct spmi_device *pdev)
{
    ...
    if((NULL == di->ops->set_runningtest)||(NULL == di->ops->set_enable_charger)
            ...
            ||(NULL == di->ops->set_chargeic_into_sleep_mode))
    {
