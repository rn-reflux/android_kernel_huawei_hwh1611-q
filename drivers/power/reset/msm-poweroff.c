...
{
#ifdef TF_SIMLOCK_ENABLED
	pr_err("dload mode is not enabled on target for TF\n");
#else
	int ret;
...
#ifdef TF_SIMLOCK_ENABLED
		} else if (!strcmp(cmd, "tf_fastboot") &&
				!strncmp(tsk->comm,"rild", 4)) {
			pr_info("current task is %s\n", tsk->comm);
			qpnp_pon_set_restart_reason(
				PON_RESTART_REASON_TF_FASTBOOT);
...
#ifdef TF_SIMLOCK_ENABLED
static ssize_t msm_dload_mode_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
{
	struct task_struct *tsk = current;

	pr_debug("Magic key code is %s\n", buf);
	if ((!strncmp(buf, "kill", 4)) && (!strncmp(tsk->comm, "rild", 4)))
		do_msm_restart(REBOOT_WARM, "tf_fastboot");
	else
		pr_notice("Not intended or authentic caller\n");

        return count;
}

DEVICE_ATTR(dload_mode, S_IWUSR, NULL, msm_dload_mode_store);
#endif
...
#ifdef TF_SIMLOCK_ENABLED
	device_create_file(&pdev->dev, &dev_attr_dload_mode);
#endif
