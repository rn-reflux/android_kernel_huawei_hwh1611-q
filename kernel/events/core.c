 ...
 */
#define DEFAULT_MAX_SAMPLE_RATE		10000
		...
		loff_t *ppos)
{
	...
	if(sysctl_perf_event_sample_rate > DEFAULT_MAX_SAMPLE_RATE )
		sysctl_perf_event_sample_rate = DEFAULT_MAX_SAMPLE_RATE;

	max_samples_per_tick = DIV_ROUND_UP(sysctl_perf_event_sample_rate, HZ);
	perf_sample_period_ns = NSEC_PER_SEC / sysctl_perf_event_sample_rate;
	update_perf_cpu_limits();

	trace_printk("SW: sysctl_perf_event_sample_rate :%d \n", sysctl_perf_event_sample_rate);
	printk("SW: sysctl_perf_event_sample_rate :%d \n", sysctl_perf_event_sample_rate);
	return 0;
}
