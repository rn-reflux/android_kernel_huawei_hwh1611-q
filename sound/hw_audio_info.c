...
#define SPEAKER_BOX_TRACFONE_NODE             "speaker-tracfone"
#define SPEAKER_BOX_TRACFONE_BUFF_SIZE        32
...
static char speaker_box_tracfone[SPEAKER_BOX_TRACFONE_BUFF_SIZE] = "tracfone";
...
static ssize_t box_tracfone_show(struct device_driver *driver, char *buf)
{
    if(NULL != buf)
    {
        snprintf(buf, PAGE_SIZE, "%s", speaker_box_tracfone);
        return strlen(buf);
    }

    return 0;
}
DRIVER_ATTR(speaker_box_tracfone, 0444, box_tracfone_show, NULL);
...
static struct attribute *audio_attrs[] = {
    ...
    &driver_attr_speaker_box_tracfone.attr,
...
static int audio_info_probe(struct platform_device *pdev)
{
    ...
    string = NULL;
    ret = of_property_read_string(pdev->dev.of_node, SPEAKER_BOX_TRACFONE_NODE, &string);
    if(ret || (NULL == string))
    {
        printk("huawei_audio: of_property_read_string speaker-tracfone failed %d\n", ret);
    }
    else
    {
        memset(speaker_box_id, 0, sizeof(speaker_box_id));
